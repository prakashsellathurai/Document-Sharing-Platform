...
pp$1.parseForStatement = function (node) {
  ...
  if (this.type === tt._var || this.type === tt._const || isLet) {
    ...
    if ((this.type === tt._in || (this.options.ecmaVersion >= 6 && this.isContextual('of'))) && init$1.declarations.length === 1 &&
        !(kind !== 'var' && init$1.declarations[0].init)) { return this.parseForIn(node, init$1) }
...
pp$1.parseLabeledStatement = function (node, maybeName, expr) {
  ...
  if (node.body.type == 'ClassDeclaration' ||
      ...
      node.body.type == 'FunctionDeclaration' && (this.strict || node.body.generator)) { this.raiseRecoverable(node.body.start, 'Invalid labeled declaration') }
  this.labels.pop()
  node.label = expr
...
pp$1.checkPatternExport = function (exports, pat) {
  ...
  if (type == 'Identifier') { this.checkExport(exports, pat.name, pat.start) } else if (type == 'ObjectPattern') {
    for (var i = 0; i < pat.properties.length; ++i) { this$1.checkPatternExport(exports, pat.properties[i].value) }
...
pp$3.parseMaybeUnary = function (refDestructuringErrors, sawUnary) {
  ...
  if (this.inAsync && this.isContextual('await')) {
    ...
    if (update) this.checkLVal(node.argument)
             ...
             node.argument.type === 'Identifier') { this.raiseRecoverable(node.start, 'Deleting local variable in strict mode') } else sawUnary = true
...
pp$3.parsePropertyValue = function (prop, isPattern, isGenerator, isAsync, startPos, startLoc, refDestructuringErrors) {
  ...
  if (this.eat(tt.colon)) {
    ...
    if (this.keywords.test(prop.key.name) ||
        ...
        (this.inAsync && prop.key.name == 'await')) { this.raiseRecoverable(prop.key.start, "'" + prop.key.name + "' can not be used as shorthand property") }
    prop.kind = 'init'
    if (isPattern) {
  ...
  if (this.type === tt.name) {
    if (!liberal && (this.strict ? this.reservedWordsStrict : this.reservedWords).test(this.value) &&
        (this.options.ecmaVersion >= 6 ||
         this.input.slice(this.start, this.end).indexOf('\\') == -1)) { this.raiseRecoverable(this.start, "The keyword '" + this.value + "' is reserved") }
...
tt._function.updateContext = function (prevType) {
  if (prevType.beforeExpr && prevType !== tt.semi && prevType !== tt._else &&
      !((prevType === tt.colon || prevType === tt.braceL) && this.curContext() === types.b_stat)) { this.context.push(types.f_expr) }
  this.exprAllowed = false
}
