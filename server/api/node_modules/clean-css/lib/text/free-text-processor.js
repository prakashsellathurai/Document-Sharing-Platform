...
function normalize (text, data, prefixContext, cursor) {
  ...
  if (lastSemicolon > -1 && lastOpenBrace > -1) { lastOne = Math.max(lastSemicolon, lastOpenBrace) } else if (lastSemicolon == -1) { lastOne = lastOpenBrace } else { lastOne = lastSemicolon }
  ...
  if (/^['"][a-zA-Z][a-zA-Z\d\-_]+['"]$/.test(text) && !/format\($/.test(context)) {
    ...
    if (isFont || isAttribute || isKeyframe || isAnimation) { text = text.substring(1, text.length - 1) }
  }
