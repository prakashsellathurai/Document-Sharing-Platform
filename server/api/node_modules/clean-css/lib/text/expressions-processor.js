...
function findEnd (data, start) {
  ...
  while (true) {
    ...
    if (quoted) {
      ...
      if (current == EXPRESSION_START) { level++ }
      if (current == EXPRESSION_END) { level-- }
      if (current == BODY_START) { braced = true }
    ...
    if (level === 0 && current == EXPRESSION_END) { break }
    if (!current) {
...
ExpressionsProcessor.prototype.escape = function (data) {
  ...
  for (; nextEnd < data.length;) {
    ...
    if (saveWaypoints) { indent = newIndent + 1 }
    cursor = nextEnd
  }
