...
function nextImportAt (data, cursor) {
  ...
  if (nextLowerCase > -1 && nextUpperCase == -1) { return nextLowerCase } else if (nextLowerCase == -1 && nextUpperCase > -1) { return nextUpperCase } else { return Math.min(nextLowerCase, nextUpperCase) }
}
...
function commentScanner (data) {
  ...
  return function scanner (idx) {
    ...
    do {
      ...
      if (idx > lastStartIndex && idx < lastEndIndex) { return true }
...
function hasContent (data) {
  ...
  while (true) {
    ...
    if (firstContentIdx == -1 || !isComment(firstContentIdx)) { break }
  }
...
function inline (data, nextStart, nextEnd, context) {
  ...
  if (isRemote && (context.localOnly || !allowedResource(importedFile, true, context.imports))) {
    if (context.afterContent || hasContent(context.done.join(''))) { context.warnings.push('Ignoring remote @import of "' + importedFile + '" as no callback given.') } else { restoreImport(importedFile, mediaQuery, context) }

    return processNext(context)
  }

  if (!isRemote && !allowedResource(importedFile, false, context.imports)) {
    if (context.afterImport) { context.warnings.push('Ignoring local @import of "' + importedFile + '" as after other inlined content.') } else { restoreImport(importedFile, mediaQuery, context) }
    return processNext(context)
  }
...
function allowedResource (importedFile, isRemote, rules) {
  ...
  if (isRemote && NO_PROTOCOL_RESOURCE.test(importedFile)) { importedFile = 'http:' + importedFile }
  ...
  for (var i = 0; i < rules.length; i++) {
    ...
    if (rule == 'all') { allowed = true } else if (isRemote && rule == 'local') { allowed = false } else if (isRemote && rule == 'remote') { allowed = true } else if (!isRemote && rule == 'remote') { allowed = false } else if (!isRemote && rule == 'local') { allowed = true } else if (rule[0] == '!' && rule.substring(1) === match) { allowed = false }
  }
...
function inlineRemoteResource (importedFile, mediaQuery, context) {
  ...
  if (NO_PROTOCOL_RESOURCE.test(importedUrl)) { importedUrl = 'http:' + importedUrl }

  if (context.visited.indexOf(importedUrl) > -1) { return processNext(context) }

  if (context.debug) { console.error('Inlining remote stylesheet: ' + importedUrl) }
  ...
  get(requestOptions, function (res) {
    ...
    res.on('end', function () {
      ...
      if (context.rebase) { importedData = rewriteUrls(importedData, { toBase: originalUrl }, context) }
      ...
      if (mediaQuery.length > 0) { importedData = '@media ' + mediaQuery + '{' + importedData + '}' }
...
function inlineLocalResource (importedFile, mediaQuery, context) {
  ...
  if (context.visited.indexOf(fullPath) > -1) { return processNext(context) }

  if (context.debug) { console.error('Inlining local stylesheet: ' + fullPath) }
