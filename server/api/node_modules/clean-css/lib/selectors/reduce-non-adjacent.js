...
function reduceComplexNonAdjacentCases (tokens, candidates, options, context) {
  ...
  for (var complexSelector in candidates) {
    ...
    for (var j = 0, m = selectors.length; j < m; j++) {
      ...
      if (stringifyBody(reducedBodies[reducedBodies.length - 1]) != stringifyBody(reducedBodies[0])) { continue allSelectors }
    }
...
function reduceSelector (tokens, selector, data, context, options, outerContext) {
  ...
  for (j = 0, m = bodiesAsList.length; j < m; j++) {
    if (bodiesAsList[j].length > 0) { joinsAt.push((joinsAt.length > 0 ? joinsAt[joinsAt.length - 1] : 0) + bodiesAsList[j].length) }
  }
