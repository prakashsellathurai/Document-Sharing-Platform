...
var valueMinifiers = {
  ...
  'font-weight': function (value) {
    if (value == 'normal') { return '400' } else if (value == 'bold') { return '700' } else { return value }
  },
...
function zeroDegMinifier (_, value) {
  if (value.indexOf('0deg') == -1) { return value }

  return value.replace(/\(0deg\)/g, '(0)')
}

function whitespaceMinifier (name, value) {
  if (name.indexOf('filter') > -1 || value.indexOf(' ') == -1) { return value }

  value = value.replace(/\s+/g, ' ')

  if (value.indexOf('calc') > -1) { value = value.replace(/\) ?\/ ?/g, ')/ ') }
...
function precisionMinifier (_, value, precisionOptions) {
  if (precisionOptions.value === -1 || value.indexOf('.') === -1) { return value }
...
function unitMinifier (name, value, unitsRegexp) {
  if (/^(?:\-moz\-calc|\-webkit\-calc|calc)\(/.test(value)) { return value }

  if (name == 'flex' || name == '-ms-flex' || name == '-webkit-flex' || name == 'flex-basis' || name == '-webkit-flex-basis') { return value }

  if (value.indexOf('%') > 0 && (name == 'height' || name == 'max-height' || name == 'width' || name == 'max-width')) { return value }
...
function multipleZerosMinifier (property) {
  ...
  if (values.length == 4 && values[0][0] === '0' && values[1][0] === '0' && values[2][0] === '0' && values[3][0] === '0') {
    if (property.name.indexOf('box-shadow') > -1) { spliceAt = 2 } else { spliceAt = 1 }
  }
...
function colorMininifier (name, value, compatibility) {
  if (value.indexOf('#') === -1 && value.indexOf('rgb') == -1 && value.indexOf('hsl') == -1) { return HexNameShortener.shorten(value) }
  ...
  value = value
    ...
    .replace(/(^|[^='"])#([0-9a-f]{6})/gi, function (match, prefix, color) {
      if (color[0] == color[1] && color[2] == color[3] && color[4] == color[5]) { return prefix + '#' + color[0] + color[2] + color[4] } else { return prefix + '#' + color }
    })
    .replace(/(rgb|rgba|hsl|hsla)\(([^\)]+)\)/g, function (match, colorFunction, colorDef) {
      ...
      if (tokens[1].indexOf('%') == -1) { tokens[1] += '%' }
      if (tokens[2].indexOf('%') == -1) { tokens[2] += '%' }
      return colorFunction + '(' + tokens.join(',') + ')'
    })

  if (compatibility.colors.opacity && name.indexOf('background') == -1) {
    value = value.replace(/(?:rgba|hsla)\(0,0%?,0%?,0\)/g, function (match) {
      if (split(value, ',').pop().indexOf('gradient(') > -1) { return match }
...
function pixelLengthMinifier (_, value, compatibility) {
  if (!WHOLE_PIXEL_VALUE.test(value)) { return value }

  return value.replace(WHOLE_PIXEL_VALUE, function (match, val) {
    var newValue
    var intVal = parseInt(val)

    if (intVal === 0) { return match }

    if (compatibility.properties.shorterLengthUnits && compatibility.units.pt && intVal * 3 % 4 === 0) { newValue = intVal * 3 / 4 + 'pt' }

    if (compatibility.properties.shorterLengthUnits && compatibility.units.pc && intVal % 16 === 0) { newValue = intVal / 16 + 'pc' }

    if (compatibility.properties.shorterLengthUnits && compatibility.units.in && intVal % 96 === 0) { newValue = intVal / 96 + 'in' }

    if (newValue) { newValue = match.substring(0, match.indexOf(val)) + newValue }
...
function minifyFont (property) {
  ...
  if (hasNumeral) { return }

  if (values[1] == '/') { return }

  var normalCount = 0
  if (values[0][0] == 'normal') { normalCount++ }
  if (values[1] && values[1][0] == 'normal') { normalCount++ }
  if (values[2] && values[2][0] == 'normal') { normalCount++ }

  if (normalCount > 1) { return }

  var toOptimize
  if (FONT_NAME_WEIGHTS_WITHOUT_NORMAL.indexOf(values[0][0]) > -1) { toOptimize = 0 } else if (values[1] && FONT_NAME_WEIGHTS_WITHOUT_NORMAL.indexOf(values[1][0]) > -1) { toOptimize = 1 } else if (values[2] && FONT_NAME_WEIGHTS_WITHOUT_NORMAL.indexOf(values[2][0]) > -1) { toOptimize = 2 } else if (FONT_NAME_WEIGHTS.indexOf(values[0][0]) > -1) { toOptimize = 0 } else if (values[1] && FONT_NAME_WEIGHTS.indexOf(values[1][0]) > -1) { toOptimize = 1 } else if (values[2] && FONT_NAME_WEIGHTS.indexOf(values[2][0]) > -1) { toOptimize = 2 }
...
function optimizeBody (properties, options) {
  ...
  for (var i = 0, l = _properties.length; i < l; i++) {
    ...
    if (property.hack && (
        ...
        property.hack == 'bang' && !options.compatibility.properties.ieBangHack)) { property.unused = true }

    if (name.indexOf('padding') === 0 && (isNegative(property, 0) || isNegative(property, 1) || isNegative(property, 2) || isNegative(property, 3))) { property.unused = true }

    if (property.unused) { continue }

    if (property.variable) {
      if (property.block) { optimizeBody(property.value[0], options) }
      continue
    }
    ...
    for (var j = 0, m = property.value.length; j < m; j++) {
      ...
      if (valueMinifiers[name]) { value = valueMinifiers[name](value, j, m) }
      ...
      if (options.compatibility.properties.colors) { value = colorMininifier(name, value, options.compatibility) }
    ...
    if (name.indexOf('border') === 0 && name.indexOf('radius') > 0) { minifyBorderRadius(property) } else if (name == 'filter') { minifyFilter(property) } else if (name == 'font') { minifyFont(property) }
  }
...
function cleanupCharsets (tokens) {
  ...
  for (var i = 0, l = tokens.length; i < l; i++) {
    ...
    if (token[0] != 'at-rule') { continue }

    if (!CHARSET_REGEXP.test(token[1][0])) { continue }
...
function buildUnitRegexp (options) {
  ...
  otherUnits.forEach(function (unit) {
    if (options.compatibility.units[unit]) { units.push(unit) }
  })
...
function optimize (tokens, options, context) {
  ...
  if (mayHaveCharset) { cleanupCharsets(tokens) }
}
