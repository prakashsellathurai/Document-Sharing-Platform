...
function mergeNonAdjacentByBody (tokens, options) {
  ...
  for (var i = tokens.length - 1; i >= 0; i--) {
    ...
    if (token[2].length > 0 && (!options.semanticMerging && unsafeSelector(stringifySelectors(token[1])))) { candidates = {} }

    if (token[2].length > 0 && options.semanticMerging && isBemElement(token)) { removeAnyUnsafeElements(token, candidates) }
