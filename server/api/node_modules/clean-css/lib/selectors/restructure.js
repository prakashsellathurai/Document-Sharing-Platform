...
function restructure (tokens, options) {
  ...
  function tokensToMerge (sourceTokens) {
    ...
    for (var i = sourceTokens.length - 1; i >= 0; i--) {
      if (isSpecial(options, stringifySelectors(sourceTokens[i][1]))) { continue }
  ...
  for (var i = tokens.length - 1; i >= 0; i--) {
    ...
    for (j = properties.length - 1; j >= 0; j--) {
      ...
      if (!isSelector || unmovableInCurrentToken.indexOf(j) > -1) { continue }
  ...
  for (; position < tokens.length - 1; position++) {
    ...
    if (!(isImportRule || isEscapedCommentSpecial)) { break }
  }
