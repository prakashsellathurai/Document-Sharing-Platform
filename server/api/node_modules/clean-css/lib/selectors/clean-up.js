...
function whitespaceReplacements (_, p1, p2, p3) {
  if (p1 && p2 && p3.length) { return p1 + p2 + ' ' } else if (p1 && p2) { return p1 + p2 } else { return p2 }
}
...
var CleanUp = {
  selectors: function (selectors, removeUnsupported, adjacentSpace) {
    ...
    for (var i = 0, l = selectors.length; i < l; i++) {
      ...
      if (adjacentSpace && reduced.indexOf('nav') > 0) { reduced = reduced.replace(/\+nav(\S|$)/, '+ nav$1') }

      if (removeUnsupported && (reduced.indexOf('*+html ') != -1 || reduced.indexOf('*:first-child+html ') != -1)) { continue }
      ...
      if (reduced.indexOf('[') > -1) { reduced = reduced.replace(/\[([^\]]+)\]/g, removeWhitespace) }
  ...
  block: function (values, spaceAfterClosingBrace) {
    ...
    if (!spaceAfterClosingBrace) { values[0] = values[0].replace(/\) /g, ')') }
  },
