...
function hasMoreProperties (tokens, index) {
  for (var i = index, l = tokens.length; i < l; i++) {
    if (typeof tokens[i] !== 'string') { return true }
  }
...
function propertyAtRule (value, isLast, context) {
  ...
  store(value, context)
  if (!isLast) { store(PROPERTY_SEPARATOR, context) }
}
...
function atRulesOrProperties (values) {
  for (var i = 0, l = values.length; i < l; i++) {
    if (values[i][0] == AT_RULE || Array.isArray(values[i][0])) { return true }
  }
