...
function whatsNext (context) {
  ...
  if (mode == 'body') {
    if (chunk[context.cursor] == '}') { return [context.cursor, 'bodyEnd'] }

    if (chunk.indexOf('}', context.cursor) == -1) { return null }
  ...
  if (nextEscape > -1 && /\S/.test(chunk.substring(context.cursor, nextEscape))) { nextEscape = -1 }

  closest = nextSpecial
  if (closest == -1 || (nextEscape > -1 && nextEscape < closest)) { closest = nextEscape }
  if (closest == -1 || (nextBodyStart > -1 && nextBodyStart < closest)) { closest = nextBodyStart }
  if (closest == -1 || (nextBodyEnd > -1 && nextBodyEnd < closest)) { closest = nextBodyEnd }

  if (closest == -1) { return }
  if (nextEscape === closest) { return [closest, 'escape'] }
  if (nextBodyStart === closest) { return [closest, 'bodyStart'] }
  if (nextBodyEnd === closest) { return [closest, 'bodyEnd'] }
  if (nextSpecial === closest) { return [closest, 'special'] }
}
  ...
  while (true) {
    ...
    if (what == 'special') {
      ...
      if (isBroken) {
        ...
        if (typeof newToken[2] === 'string') { newToken[2] = extractProperties(newToken[2], [[trimmedValue]], context) }
      ...
      if (isStartSourceMarker) {
        ...
        if (escaped.indexOf('__ESCAPED_COMMENT_SPECIAL') === 0) { tokenized.push(['text', [escaped]]) }
      ...
      if (context.mode == 'block') { context.track(chunk.substring(context.cursor, nextSpecial)) }
      if (context.mode != 'block') { tokenized = chunk.substring(context.cursor, nextSpecial) }
