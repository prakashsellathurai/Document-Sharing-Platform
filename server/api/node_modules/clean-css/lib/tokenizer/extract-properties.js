...
function extractProperties (string, selectors, context) {
  ...
  if (string.indexOf(')') > -1) { string = string.replace(/\)([^\s_;:,\)])/g, context.sourceMap ? ') __ESCAPED_COMMENT_CLEAN_CSS(0,-1)__ $1' : ') $1') }

  if (string.indexOf('ESCAPED_URL_CLEAN_CSS') > -1) { string = string.replace(/(ESCAPED_URL_CLEAN_CSS[^_]+?__)/g, context.sourceMap ? '$1 __ESCAPED_COMMENT_CLEAN_CSS(0,-1)__ ' : '$1 ') }
  ...
  for (var i = 0, l = candidates.length; i < l; i++) {
    ...
    if (firstColonAt == -1) {
      context.track(candidate)
      if (candidate.indexOf('__ESCAPED_COMMENT_SPECIAL') > -1) { list.push(candidate.trim()) }
      continue
    }
    ...
    if (name.indexOf('__ESCAPED_COMMENT') > -1) { name = withoutHeadingComments(name, list, context) }

    if (name.indexOf('__ESCAPED_COMMENT') > -1) { name = withoutInnerComments(name, innerComments, context) }
    ...
    for (var j = 0, m = values.length; j < m; j++) {
      ...
      if (endsWithNonSpaceSeparator) { trimmed = trimmed.substring(0, trimmed.length - 1) }
      ...
      if (trimmed.indexOf('__ESCAPED_COMMENT') > -1) { trimmed = withoutHeadingComments(trimmed, list, context) }

      if (trimmed.indexOf('__ESCAPED_COMMENT') > -1) { trimmed = withoutInnerComments(trimmed, innerComments, context) }
