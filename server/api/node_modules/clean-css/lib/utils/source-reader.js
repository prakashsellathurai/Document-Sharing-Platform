...
SourceReader.prototype.toString = function () {
  if (typeof this.data === 'string') { return fromString(this) }
  if (Buffer.isBuffer(this.data)) { return fromBuffer(this) }
  if (Array.isArray(this.data)) { return fromArray(this) }
...
function fromHash (self) {
  ...
  for (var source in self.data) {
    ...
    if (self.outerContext.options.sourceMap && inputSourceMap) { self.outerContext.inputSourceMapTracker.trackLoaded(source, source, inputSourceMap) }
