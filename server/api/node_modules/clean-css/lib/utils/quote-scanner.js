...
var findQuoteEnd = function (data, matched, cursor, oldCursor) {
  ...
  if (commentEndedAt >= cursor && commentStartedAt > -1) { commentStarted = true }
  if (commentStartedAt < cursor && commentStartedAt > commentEndedAt) { commentStarted = true }

  if (commentStarted) {
    var commentEndsAt = data.indexOf(commentEndMark, cursor)
    if (commentEndsAt > -1) { return commentEndsAt }
  ...
  while (true) {
    ...
    if (data[cursor] == matched && (data[cursor - 1] != escapeMark || data[cursor - 2] == escapeMark)) { break }
...
function findNext (data, mark, startAt) {
  ...
  while (true) {
    ...
    if (data[candidate - 1] != escapeMark) { return candidate }
  }
}
...
QuoteScanner.prototype.each = function (callback) {
  ...
  for (; nextEnd < data.length;) {
    ...
    if (nextStartSingle == -1) { nextStartSingle = dataLength }
    if (nextStartDouble == -1) { nextStartDouble = dataLength }
    ...
    if (text.length > 0) { callback(text, tempData, nextStart) }
