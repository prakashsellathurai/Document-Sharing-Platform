...
function fromSource (self, data, whenDone, context) {
  ...
  while (context.cursor < data.length) {
    ...
    nextAt = data.length
    if (markerStartMatch.index > -1) { nextAt = markerStartMatch.index }
    if (markerEndMatch.index > -1 && markerEndMatch.index < nextAt) { nextAt = markerEndMatch.index }
    if (mapMatch.index > -1 && mapMatch.index < nextAt) { nextAt = mapMatch.index }

    if (nextAt == data.length) { break }
...
function fetch (self, path, onSuccess, onFailure) {
  ...
  protocol
    .get(requestOptions, function (res) {
      if (res.statusCode < 200 || res.statusCode > 299) { return onFailure(res.statusCode) }
...
function originalPositionIn (trackedSource, line, column, token, allowNFallbacks) {
  ...
  if (originalPosition.line === null && line > 1 && allowNFallbacks > 0) { return originalPositionIn(trackedSource, line - 1, column, token, allowNFallbacks - 1) }
...
function _resolveSources (self, remaining, whenDone) {
  ...
  if (remaining.length === 0) { return whenDone() }
  ...
  if (isRemote) {
    ...
    if (fs.existsSync(fullPath)) { self.sourcesContent[sourceFile][originalFile] = fs.readFileSync(fullPath, 'utf-8') } else { self.warnings.push('Missing original source file at "' + fullPath + '".') }
    return processNext()
  }
}
...
InputSourceMapStore.prototype.resolveSources = function (whenDone) {
  ...
  for (var sourceFile in this.sourcesContent) {
    ...
    for (var originalFile in contents) {
      if (!contents[originalFile]) { toResolve.push([sourceFile, originalFile]) }
    }
  }
