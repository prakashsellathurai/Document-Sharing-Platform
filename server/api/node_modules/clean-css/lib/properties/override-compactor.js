...
function wouldBreakCompatibility (property, validator) {
  for (var i = 0; i < property.components.length; i++) {
    ...
    if (!canOverride(_component, component, validator)) { return true }
  }
...
function turnIntoMultiplex (property, size) {
  ...
  for (var i = 0, l = property.components.length; i < l; i++) {
    var component = property.components[i]
    if (component.multiplex) { continue }
...
function multiplexSize (component) {
  ...
  for (var i = 0, l = component.value.length; i < l; i++) {
    if (component.value[i][0] == MULTIPLEX_SEPARATOR) { size++ }
  }
...
function overridingFunction (shorthand, validator) {
  for (var i = 0, l = shorthand.components.length; i < l; i++) {
    if (anyValue(validator.isValidFunction, shorthand.components[i])) { return true }
  }
...
function anyValue (fn, property) {
  for (var i = 0, l = property.value.length; i < l; i++) {
    if (property.value[i][0] == MULTIPLEX_SEPARATOR) { continue }

    if (fn(property.value[i][0])) { return true }
  }
...
function anyLayerIsNone (values) {
  ...
  for (var i = 0, l = layers.length; i < l; i++) {
    if (layers[i].length == 1 && layers[i][0][0] == 'none') { return true }
  }
...
function compactOverrides (properties, compatibility, validator) {
  ...
  for (i = properties.length - 1; i >= 0; i--) {
    ...
    for (j = i - 1; j >= 0; j--) {
      ...
      if (left.hack && !right.hack || !left.hack && right.hack) { continue }
      ...
      if (!left.shorthand && right.shorthand && isComponentOf(right, left)) {
        ...
        if (!anyValue(validator.isValidFunction, left) && overridingFunction(right, validator)) { continue }
        ...
        if (everyCombination(mayOverride, component, right, validator)) {
          ...
          if (component.value[0][0] != right.value[0][0] && (hasInherit(left) || hasInherit(right))) { continue }
        ...
        for (k = left.components.length - 1; k >= 0; k--) {
          ...
          if (!everyCombination(mayOverride, leftComponent, rightComponent, validator)) { continue propertyLoop }
          if (!everyCombination(canOverride.twoOptionalFunctions, leftComponent, rightComponent, validator) && validator.isValidFunction(rightComponent)) { continue propertyLoop }
        }
        ...
        component = left.components.filter(nameMatchFilter(right))[0]
        mayOverride = compactable[right.name].canOverride || canOverride.sameValue
        if (!everyCombination(mayOverride, component, right, validator)) { continue }
        ...
        mayOverride = compactable[right.name].canOverride || canOverride.sameValue
        if (!everyCombination(mayOverride, left, right, validator)) { continue }
