...
function wrapAll (properties) {
  ...
  for (var i = properties.length - 1; i >= 0; i--) {
    if (typeof properties[i][0] === 'string') { continue }
...
function isMultiplex (property) {
  for (var i = 1, l = property.length; i < l; i++) {
    if (property[i][0] == ',' || property[i][0] == '/') { return true }
  }
...
function stripImportant (property) {
  if (property.length > 0) { property[property.length - 1][0] = property[property.length - 1][0].replace(IMPORTANT_TOKEN_MATCH, '') }
}
...
function stripSuffixHack (property, hackType) {
  ...
  if (lastValue[0].length === 0) { property.pop() }
}

function wrapSingle (property) {
  var _isImportant = isImportant(property)
  if (_isImportant) { stripImportant(property) }

  var _hackType = hackType(property)
  if (_hackType == 'star' || _hackType == 'underscore') { stripPrefixHack(property) } else if (_hackType == 'backslash' || _hackType == 'bang') { stripSuffixHack(property, _hackType) }
