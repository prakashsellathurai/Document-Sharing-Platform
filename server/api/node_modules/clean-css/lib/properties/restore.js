...
function isInheritOnly (values) {
  for (var i = 0, l = values.length; i < l; i++) {
    ...
    if (value != 'inherit' && value != MULTIPLEX_SEPARATOR && value != SIZE_POSITION_SEPARATOR) { return false }
  }
...
function background (property, compactable, lastInMultiplex) {
  ...
  function isDefaultValue (component) {
    ...
    if (descriptor.doubleValues) {
      if (descriptor.defaultValue.length == 1) { return component.value[0][0] == descriptor.defaultValue[0] && (component.value[1] ? component.value[1][0] == descriptor.defaultValue[0] : true) } else { return component.value[0][0] == descriptor.defaultValue[0] && (component.value[1] ? component.value[1][0] : component.value[0][0]) == descriptor.defaultValue[1] }
    } else {
  ...
  if (restored.length === 0 && property.value.length == 1 && property.value[0][0] == '0') { restored.push(property.value[0]) }

  if (restored.length === 0) { restored.push([compactable[property.name].defaultValue]) }
...
function multiplex (restoreWith) {
  return function (property, compactable) {
    ...
    for (i = 0, l = property.components[0].value.length; i < l; i++) {
      if (property.components[0].value[i][0] == MULTIPLEX_SEPARATOR) { multiplexSize++ }
    }
    ...
    for (i = 0; i <= multiplexSize; i++) {
      ...
      if (i < multiplexSize) { restored.push([',']) }
    }
...
function withoutDefaults (property, compactable) {
  ...
  for (var i = components.length - 1; i >= 0; i--) {
    ...
    if (component.value[0][0] != descriptor.defaultValue) { restored.unshift(component.value[0]) }
  }
