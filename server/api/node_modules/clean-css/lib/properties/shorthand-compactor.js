...
function mixedImportance (components) {
  ...
  for (var name in components) {
    if (undefined !== important && components[name].important != important) { return true }
...
function componentSourceMaps (components) {
  ...
  for (var name in components) {
    ...
    if (Array.isArray(mapping)) { Array.prototype.push.apply(sourceMapping, mapping) }
  }
...
function replaceWithShorthand (properties, candidateComponents, name, sourceMaps, validator) {
  ...
  for (var i = 0, l = descriptor.components.length; i < l; i++) {
    ...
    if (hasInherit(component)) { return }

    if (!everyCombination(canOverride, newProperty.components[i], component, validator)) { return }
  ...
  if (sourceMaps) {
    ...
    if (sourceMapping.length > 0) { newValuePlaceholder[0].push(sourceMapping) }
  }
...
function invalidateOrCompact (properties, position, candidates, sourceMaps, validator) {
  ...
  for (var name in candidates) {
    if (undefined !== property && name == property.name) { continue }
    ...
    if (mixedImportance(candidateComponents)) { continue }
...
function compactShortands (properties, sourceMaps, validator) {
  ...
  for (var i = 0, l = properties.length; i < l; i++) {
    ...
    if (property.hack) { continue }

    if (property.variable) { continue }

    var descriptor = compactable[property.name]
    if (!descriptor || !descriptor.componentOf) { continue }
