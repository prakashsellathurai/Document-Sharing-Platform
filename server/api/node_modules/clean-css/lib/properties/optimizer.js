...
function _optimize (properties, mergeAdjacent, aggressiveMerging, validator) {
  ...
  function mergeablePosition (position) {
    if (mergeAdjacent === false || mergeAdjacent === true) { return mergeAdjacent }
  ...
  for (var position = 0, total = properties.length; position < total; position++) {
    ...
    if (_name in overrideMapping && (aggressiveMerging && _name != lastName || mergeablePosition(position))) {
      ...
      for (j = toOverridePositions.length - 1; j >= 0; j--) {
        ...
        if (toRemove.unused) { continue }

        if (longhandToShorthand && wasImportant) { continue }

        if (!wasImportant && (wasHack && !isHack || !wasHack && isHack)) { continue }

        if (wasImportant && (isHack == 'star' || isHack == 'underscore')) { continue }

        if (!wasHack && !isHack && !longhandToShorthand && canOverride && !canOverride(toRemove, property, validator)) { continue }
...
function optimize (selector, properties, mergeAdjacent, withCompacting, options, context) {
  ...
  for (var i = 0, l = _properties.length; i < l; i++) {
    ...
    if (_property.variable && _property.block) { optimize(selector, _property.value[0], mergeAdjacent, withCompacting, options, context) }
  }
