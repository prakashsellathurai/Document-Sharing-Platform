...
function byUrl (data, context, callback) {
  ...
  for (; nextEnd < data.length;) {
    ...
    if (nextStart == -1 && nextStartUpperCase == -1) { break }

    if (nextStart == -1 && nextStartUpperCase > -1) { nextStart = nextStartUpperCase }
    ...
    if (nextEnd == -1) {
      ...
      if (nextEnd == -1) { nextEnd = data.length } else { nextEnd-- }

      context.warnings.push('Broken URL declaration: \'' + data.substring(nextStart, nextEnd + 1) + '\'.')
    } else {
      if (data[nextEnd] != URL_SUFFIX) { nextEnd = data.indexOf(URL_SUFFIX, nextEnd) }
    }
...
function byImport (data, context, callback) {
  ...
  for (; nextEnd < data.length;) {
    ...
    if (nextImport == -1 && nextImportUpperCase == -1) { break }

    if (nextImport > -1 && nextImportUpperCase > -1 && nextImportUpperCase < nextImport) { nextImport = nextImportUpperCase }
