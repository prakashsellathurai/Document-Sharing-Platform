...
function rebase (uri, options) {
  if (isAbsolute(uri) || isSVGMarker(uri) || isEscaped(uri) || isInternal(uri)) { return uri }

  if (options.rebase === false && !isImport(uri)) { return uri }

  if (!options.imports && isImport(uri)) { return uri }

  if (isData(uri)) { return '\'' + uri + '\'' }

  if (isRemote(uri) && !isRemote(options.toBase)) { return uri }

  if (isRemote(uri) && !isSameOrigin(uri, options.toBase)) { return uri }

  if (!isRemote(uri) && isRemote(options.toBase)) { return url.resolve(options.toBase, uri) }
...
function quoteFor (url) {
  if (url.indexOf('\'') > -1) { return '"' } else if (url.indexOf('"') > -1) { return '\'' } else if (/\s/.test(url) || /[\(\)]/.test(url)) { return '\'' } else { return '' }
}
