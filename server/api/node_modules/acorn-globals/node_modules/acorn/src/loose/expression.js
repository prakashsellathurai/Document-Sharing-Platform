...
lp.parseSubscripts = function (base, start, noCalls, startIndent, line) {
  for (;;) {
    ...
    if (this.eat(tt.dot)) {
      ...
      if (maybeAsyncArrow && this.eat(tt.arrow)) { return this.parseArrowExpression(this.startNodeAt(start), exprList, true) }
      let node = this.startNodeAt(start)
      node.callee = base
  ...
  switch (this.tok.type) {
    ...
    case tt.name:
      ...
      if (id.name === 'async' && !this.canInsertSemicolon()) {
        if (this.eat(tt._function)) { return this.parseFunction(this.startNodeAt(start), false, true) }
...
lp.toAssignable = function (node, binding) {
  if (!node || node.type == 'Identifier' || (node.type == 'MemberExpression' && !binding)) {
    ...
    for (let i = 0; i < props.length; i++) { props[i].value = this.toAssignable(props[i].value, binding) }
...
lp.toAssignableList = function (exprList, binding) {
  for (let i = 0; i < exprList.length; i++) { exprList[i] = this.toAssignable(exprList[i], binding) }
  return exprList
}
