...
function makeTest (test) {
  if (typeof test === 'string') { return type => type == test } else if (!test) { return () => true } else { return test }
}
  ...
  try {
    ;(function c (node, st, override) {
      ...
      if ((start == null || node.start <= start) &&
          (end == null || node.end >= end)) { base[type](node, st, c) }
      if ((start == null || node.start == start) &&
          (end == null || node.end == end) &&
          test(type, node)) { throw new Found(node, st) }
    })(node, state)
  } catch (e) {
...
base.VariableDeclaration = (node, st, c) => {
  for (let i = 0; i < node.declarations.length; ++i) { c(node.declarations[i], st) }
}
...
base.Function = (node, st, c) => {
  ...
  for (let i = 0; i < node.params.length; i++) { c(node.params[i], st, 'Pattern') }
...
base.ObjectPattern = (node, st, c) => {
  for (let i = 0; i < node.properties.length; ++i) { c(node.properties[i].value, st, 'Pattern') }
}
...
base.ObjectExpression = (node, st, c) => {
  for (let i = 0; i < node.properties.length; ++i) { c(node.properties[i], st) }
}
base.FunctionExpression = base.ArrowFunctionExpression = base.FunctionDeclaration
base.SequenceExpression = base.TemplateLiteral = (node, st, c) => {
  for (let i = 0; i < node.expressions.length; ++i) { c(node.expressions[i], st, 'Expression') }
}
...
base.NewExpression = base.CallExpression = (node, st, c) => {
  ...
  if (node.arguments) {
    for (let i = 0; i < node.arguments.length; ++i) { c(node.arguments[i], st, 'Expression') }
  }
}
...
base.ImportDeclaration = (node, st, c) => {
  for (let i = 0; i < node.specifiers.length; i++) { c(node.specifiers[i], st) }
  c(node.source, st, 'Expression')
}
...
base.Class = (node, st, c) => {
  ...
  for (let i = 0; i < node.body.body.length; i++) { c(node.body.body[i], st) }
}
