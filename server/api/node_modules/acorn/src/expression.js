...
pp.parsePropertyValue = function (prop, isPattern, isGenerator, startPos, startLoc, refDestructuringErrors) {
  if (this.eat(tt.colon)) {
    ...
    if (this.keywords.test(prop.key.name) ||
        ...
        (this.inGenerator && prop.key.name == 'yield')) { this.raiseRecoverable(prop.key.start, "'" + prop.key.name + "' can not be used as shorthand property") }
    prop.kind = 'init'
    if (isPattern) {
...
pp.checkParams = function (node, useStrict) {
  ...
  for (let i = 0; i < node.params.length; i++) {
    if (useStrict && this.options.ecmaVersion >= 7 && node.params[i].type !== 'Identifier') { this.raiseRecoverable(useStrict.start, "Illegal 'use strict' directive in function with non-simple parameter list") }
...
pp.parseExprList = function (close, allowTrailingComma, allowEmpty, refDestructuringErrors) {
  ...
  while (!this.eat(close)) {
    ...
    let elt
    if (allowEmpty && this.type === tt.comma) { elt = null } else if (this.type === tt.ellipsis) {
