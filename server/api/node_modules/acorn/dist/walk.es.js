...
function makeTest (test) {
  if (typeof test === 'string') { return function (type) { return type == test } } else if (!test) { return function () { return true } } else { return test }
}
...
base.VariableDeclaration = function (node, st, c) {
  for (var i = 0; i < node.declarations.length; ++i) { c(node.declarations[i], st) }
}
...
base.Function = function (node, st, c) {
  ...
  for (var i = 0; i < node.params.length; i++) { c(node.params[i], st, 'Pattern') }
...
base.ObjectPattern = function (node, st, c) {
  for (var i = 0; i < node.properties.length; ++i) { c(node.properties[i].value, st, 'Pattern') }
}
...
base.NewExpression = base.CallExpression = function (node, st, c) {
  ...
  if (node.arguments) {
    for (var i = 0; i < node.arguments.length; ++i) { c(node.arguments[i], st, 'Expression') }
  }
}
...
base.ImportDeclaration = function (node, st, c) {
  for (var i = 0; i < node.specifiers.length; i++) { c(node.specifiers[i], st) }
  c(node.source, st, 'Expression')
}
