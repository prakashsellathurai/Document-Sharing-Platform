...
var giveMe = {
  ...
  iterate: function (parameters) {
    ...
    if (tasks.length == 0) { return callback([]) }

    var callbacks = new CallbacksManager(tasks, options)

    if (options.callbackMethod == 'sequence') { this.runSequenceTask(tasks, 0, 1, callbacks, callback) } else
      if (!maxConcurrency) {
        for (var i = 0; i < tasks.length; i++) { this.runTask(tasks[i], callbacks, callback) }
      } else {
        for (var i = 0; i < Math.min(tasks.length, maxConcurrency); i++) { this.runSequenceTask(tasks, i, maxConcurrency, callbacks, callback) }
      }
  },
