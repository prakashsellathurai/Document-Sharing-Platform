  ...
  this.tryExecute = function (callback) {
    ...
    var satisfied = {
      ...
      conditional: function () {
        if (!options.conditionalFunction || typeof (options.conditionalFunction) !== 'function') { return true }

        for (var i = 0; i < output.length; i++) {
          if (options.conditionalFunction(output[i]) == true) { return true }
        }
    ...
    var tuplifyCallback = function (callbacks) {
      ...
      for (var i = 0; i < callbacks.length; i++) {
        ...
        errors.push(cb[0])
        results.push(cb[1])
        if (typeof (cb[0]) === 'undefined' || cb[0] === null) { errorsNullsOrUndefineds++ }
      };
